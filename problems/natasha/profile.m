...
%PROFILE Sweep definitions and evaluators for Natasha.
    ...
    mValues = [0, 0.1, 0.2];
    lambdaVals = [-3.2,-2.733333333,-2.266666667,-1.8,-1.333333333,-0.866666667,-0.4,0.066666667,0.533333333,1];
...
function configs = buildFigureConfigs(cfg, metricEvaluators)
    ...
    configs = [
        struct('mode','profile','figureId',1,'componentIdx',2,'domainWindow',[0,20],'codomainWindow',[],'numSamples',800, ...
               'xlabel','domain','ylabel','codomain','title','Figure 1 (M sweep)', ...
               'metricField','Cf_star','metricFn',[],'branchIdx',1:2,'useSharedColor',false);
        struct('mode','profile','figureId',2,'componentIdx',4,'domainWindow',[0,0.25],'codomainWindow',[0,0.02],'numSamples',801, ...
               'xlabel','domain','ylabel','codomain','title','Figure 2 (M sweep)', ...
               'metricField','Nu_star','metricFn',[],'branchIdx',1:2,'useSharedColor',false);
        struct('mode','metric','figureId',3,'componentIdx',[],'domainWindow',[0,20],'codomainWindow',[], 'numSamples',800, ...
        ...
        struct('mode','metric','figureId',4,'componentIdx',[],'domainWindow',[0,0.25],'codomainWindow',[],'numSamples',800, ...
